...
**Freeze record:** date 2026-09-23, commit `22fc9211409aff8c89ea8bae23466a6753c67f16` (the PLAYBOOK/ROADMAP/REPORT_TEMPLATE commit, per operator instruction to date the freeze on that commit — filled into `docs/METHOD_V2.md`'s header in the next commit, `4835af9`, since the SHA could not be known before that commit existed). **First eligible event is UNVERIFIED, not resolved.** Initial web-search pass (09-23) suggested "PGL Fall 2026" (S-Tier, 2026-10-01–10-11), starting before ESL Pro League S24 (10-03) — but a second pass (09-24, prompted by the operator: "to może być inny turniej, muszę sprawdzić co grają") found this name **absent from PGL's own official event listing** (which shows Bucharest, Cluj-Napoca, Astana, PGL Masters Bucharest 22–31.10, Major Singapore — no "Fall"), while search kept surfacing a Liquipedia URL and qualifier details for it. Direct fetch to both Liquipedia and HLTV is blocked in this session, so the repo agent cannot resolve the contradiction with available tools. **Operator must confirm the actual first tier-1 event before 2026-10-01** — see `docs/decisions/2026-09-23-v2-freeze.md` for the full trail. Full method: `docs/METHOD_V2.md`. Approval record: `docs/decisions/2026-09-23-v2-freeze.md`.
