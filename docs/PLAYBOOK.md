...
- **Shadow runs.** A new version may run as a *shadow* (paper-only) valuation alongside the active version on the same matches, so the old version reaches its own checkpoint and the two are compared on an identical paired sample rather than across different periods. A shadow entry never drives real money: it carries its `method_version` **and** a `mode: "shadow"` flag, is excluded from operator P&L and the shared CLV pool, and is settled and counted only within its own version's sample (sharing matches does not let it inherit another version's sample). Paired shadow/official entries must share `match`, `date`, `odds_timestamp`, and odds snapshot so the paired comparison is computable. The launch path and reporting shape are described in ROADMAP → Method versioning path.
